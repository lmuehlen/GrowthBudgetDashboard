...
# Haushaltsanalyse
```{r}
HaushaltsanalysePcBIP<-read_excel("Data/Haushaltsanalyse v10.xlsx",sheet = 1,range = "A1:P71")%>% #adjust range if column added
  filter(!is.na(Aggregat))%>%
  select(-'ID/Code')

HaushaltsanalyseMrdEur<-read_excel("Data/Haushaltsanalyse v10.xlsx",sheet = 2,range = "A1:P71")%>% #adjust range if column added
  filter(!is.na(Aggregat))%>%
  select(-'ID/Code')
...
```{r}
#TODO: make English
plotBundesausgaben<-function(data,filename,path="output/Juni2025",language="DE",with=577.7,height=519.75){
BundesausgabenData<-data%>%
  filter(Aggregat=="Gesamtausgaben des Bundes")%>%
  mutate(
    variable=plyr::revalue(Titel,c("Sonstiges"="Sonstiges\n (inkl. globale Minderausgabe)",
                                   "Verwaltung (inkl. Sicherheitsbehörden)"="Verwaltung\n (inkl. Sicherheitsbehörden)",
                                "Historische Ereignisse & Altlasten"="Hist. Ereignisse & Altlasten",
                                "Zinsen"="Zinsen",
                                "Internationales"="Internationales",
                                "Verteidigung"="Verteidigung",
                                "Öffentliche Leistungen"="Öff. Leistungen",
                                "Subventionen an Haushalte & Unternehmen"="Subventionen\n (Haushalte & Unternehmen)"
                                )),
...
BundesausgabenPlotly<-plot_ly(
  ...
  y           = ~PcBIP,
  color       = ~variable,
  colors      = c("#BFBFBF","#ee6174","#333C91","#88536f","#36A9E1","#F39433","#918AB2","#BE4776"),
  text        = ~tooltip,
  hoverinfo   = "text",
  hovertemplate = "%{text}<extra></extra>",
  type        = "scatter",
...
htmlwidgets::saveWidget(widget = BundesausgabenPlotly, file=paste0(path,filename,".html"),selfcontained = FALSE,libdir = "Plotlydependencies")
}
```

## Bundeseinnahmen
```{r}
BundeseinnahmenData<-HaushaltsanalyseData%>%
  filter(Aggregat=="Gesamteinnahmen des Bundes")%>%
  mutate(
    variable=factor(Titel,levels=c("Umsatz- & Einfuhrumsatzsteuer",
...
BundeseinnahmenPlotly<-plot_ly(
  ...
  y           = ~PcBIP,
  color       = ~variable,
  colors      = c("#ee6174","#333C91","#88536f","#36A9E1","#F39433","#918AB2","#BE4776","#BFBFBF"),
  text        = ~tooltip,
  hoverinfo   = "text",
  hovertemplate = "%{text}<extra></extra>",
  type        = "scatter",
...
htmlwidgets::saveWidget(widget = BundeseinnahmenPlotly, file="Bundeseinnahmen.html",selfcontained = FALSE,libdir = "Plotlydependencies")
```
...
htmlwidgets::saveWidget(widget = BundesinvestitionenPlotly, file="Bundesinvestitionen.html",selfcontained = FALSE,libdir = "Plotlydependencies")
```

## Öffentliche Leistungen des Bundes
```{r}
ÖffentlicheLeistungenData<-HaushaltsanalyseData%>%
...
htmlwidgets::saveWidget(widget = ÖffentlicheLeistungenPlotly, file="ÖffentlicheLeistungen.html",selfcontained = FALSE,libdir = "Plotlydependencies")
```

## Subventionen des Bundes an Haushalte
```{r}
SubventionenHaushalteData<-HaushaltsanalyseData%>%
...
htmlwidgets::saveWidget(widget = SubventionenHaushaltePlotly, file="SubventionenHaushalte.html",selfcontained = FALSE,libdir = "Plotlydependencies")
```


## Subventionen des Bundes an Unternehmen
```{r}
SubventionenUnternehmenData<-HaushaltsanalyseData%>%
  ...
  mutate(
    variable=plyr::revalue(Titel,c("Strompreise"="Strompreise",
                                   ...
                                   "Sonstige Innovation & Forschung"="Sonst. Innovation & Forschung",
                                   "Land- & Forstwirtschaft"="Land- & Forstwirtschaft",
                                   "Sonstiges"="Sonstiges"
                                   )),
    variable=factor(variable,levels=c("Strompreise",
                                   "Digitales",
                                   "Klima & Energie\n (ohne Strompreise)",
...
SubventionenUnternehmenPlotly<-plot_ly(
  ...
  y           = ~PcBIP,
  color       = ~variable,
  colors      = c("#ee6174","#333C91","#88536f","#36A9E1","#F39433","#BE4776","#A65FBD","#BFBFBF"),
  text        = ~tooltip,
  hoverinfo   = "text",
  hovertemplate = "%{text}<extra></extra>",
  type        = "scatter",
...
htmlwidgets::saveWidget(widget = SubventionenUnternehmenPlotly, file="SubventionenUnternehmen.html",selfcontained = FALSE,libdir = "Plotlydependencies")
```
